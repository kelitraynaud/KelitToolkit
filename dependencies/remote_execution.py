...
# From the MIT-licensed BlenderTools project (poly-hammer/BlenderTools) -
# see LICENSE-BlenderTools.txt in this folder for the full MIT notice.
...
class RemoteExecutionConfig(object):
    ...
    Vendored into Kelit Toolkit with fixed defaults: Unreal's standard remote-execution
    settings (Project Settings > Plugins > Python), which is what the vast
    majority of projects use unchanged.
    '''
    def __init__(self):
        # multicast time-to-live (0 = local machine only, matches UE default)
        self.multicast_ttl = 0

        # UDP multicast group the Python plugin broadcasts on
        self.multicast_group_endpoint = ('239.0.0.1', 6766)

        # TCP command endpoint this client hosts for the editor to connect back to
        self.command_endpoint = ('127.0.0.1', 6776)

        # adapter the UDP multicast socket binds to. Must be a local address,
        # not the multicast group (the
        # command endpoint host), not the group address.
        self.multicast_bind_address = '127.0.0.1'
...
class _RemoteExecutionCommandConnection(object):
    ...
    def _receive_message(self, expected_type):
        ...
        B2UE patch: a command result larger than one TCP segment arrives
        fragmented, and the original single recv() truncated it - the JSON
        then failed to parse and the whole call surfaced as "no editor
        found". Accumulate segments until the JSON is complete. The first
        recv stays fully blocking (commands can legitimately run for
        minutes); a timeout only guards the gaps between fragments of a
        message that has already started arriving.
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # incomplete message: keep reading, but never hang forever
                    # on a half-sent payload
                    self._command_channel_socket.settimeout(10)
                    continue
                message = _RemoteExecutionMessage(None, None)
